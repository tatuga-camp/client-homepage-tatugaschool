...
export default function Home() {
  ...
  const features = [
    {
      title: HomeDataLanguage.features_unlimited_storage_title(currentLanguage),
      description:
        HomeDataLanguage.features_unlimited_storage_desc(currentLanguage),
      image: "/images/features/1.png",
    },
    {
      title:
        HomeDataLanguage.features_predictive_analysis_title(currentLanguage),
      description:
        HomeDataLanguage.features_predictive_analysis_desc(currentLanguage),
      image: "/images/features/2.png",
    },
    {
      title: HomeDataLanguage.features_no_login_title(currentLanguage),
      description: HomeDataLanguage.features_no_login_desc(currentLanguage),
      image: "/images/features/3.png",
    },
    {
      title: HomeDataLanguage.features_gamification_title(currentLanguage),
      description: HomeDataLanguage.features_gamification_desc(currentLanguage),
  ...
  const sponsors = [
    {
      title: "TedFund",
      description: HomeDataLanguage.tedfund_desc(currentLanguage),
      image:
        "https://tedfund.mhesi.go.th/images/asset/logo/TED_Youth_Startup_LOGO.png",
    },
    {
      title: "NRRU UBI",
      description: HomeDataLanguage.nrru_ubi_desc(currentLanguage),
    ...
    <Layout>
      ...
      >
        <section className="w-96 md:w-8/12 p-2 text-center items-center  md:max-w-[30rem] flex flex-col gap-1">
          ...
          >
            {HomeDataLanguage.students_go(currentLanguage)}
          </a>
          ...
          <p className="text-gray-500 font-medium">
            {HomeDataLanguage.description(currentLanguage)}
          </p>
        </section>
      ...
      <main className="my-20 flex flex-col gap-10 font-Anuphan">
        ...
        >
          <p className="xl:col-span-2 text-base md:text-xs lg:text-base font-semibold text-center flex items-center px-10 md:px-0 ">
            {HomeDataLanguage.nav_text(currentLanguage)}
          </p>
          <section className="flex col-span-1 flex-col items-center justify-center gap-1">
            <h1 className="text-4xl font-bold">2,136+</h1>
            <span>{HomeDataLanguage.number_school(currentLanguage)}</span>
          </section>
          <section className="flex col-span-1 flex-col items-center justify-center gap-1">
            <h1 className="text-4xl font-bold">3,793+</h1>
            <span>{HomeDataLanguage.number_user(currentLanguage)}</span>
          </section>
          <section className="flex col-span-1 flex-col items-center justify-center gap-1">
            <h1 className="text-4xl font-bold">57,268+</h1>
            <span>{HomeDataLanguage.number_student(currentLanguage)}</span>
          </section>
        </nav>
        <section className="w-full flex items-center md:flex-row flex-col justify-center gap-10">
          ...
          <div className="w-80 md:w-96 flex flex-col items-start justify-center gap-1">
            ...
            <h1 className="text-xl text-black font-bold">
              {HomeDataLanguage.manage_desc(currentLanguage)}
            </h1>
            <p className="text-gray-500 text-sm mt-2 font-medium">
              {HomeDataLanguage.join_us(currentLanguage)}
            </p>
            <Link
              href={
            ...
            >
              {HomeDataLanguage.learn_more(currentLanguage)}
            </Link>
          </div>
        </section>
        <section className="w-full p-5 md:px-10 grid gap-5 grid-cols-1 md:grid-cols-3 xl:grid-cols-4">
          <div className="flex flex-col gap-1 justify-center items-start">
            <h1 className="text-sm text-orange-500 font-bold">
              {HomeDataLanguage.feature(currentLanguage)}
            </h1>
            <h1 className="text-3xl text-blue-600 font-bold">
              {HomeDataLanguage.what_you_can_do(currentLanguage)}
            </h1>
            <p className="text-gray-500 text-sm">
              {HomeDataLanguage.feature_desc(currentLanguage)}
            </p>
          </div>
          <Swiper
            slidesPerView={3}
        ...
        <section className="w-full p-5 md:px-10 grid gap-5 grid-cols-1 md:grid-cols-3 xl:grid-cols-4">
          <div className="flex flex-col gap-1 justify-center items-start">
            <h1 className="text-sm text-orange-500 font-bold">
              {HomeDataLanguage.sponsors(currentLanguage)}
            </h1>
            <h1 className="text-3xl text-blue-600 font-bold">
              {HomeDataLanguage.beloved_sponsors(currentLanguage)}
            </h1>
            <p className="text-gray-500 text-sm">
              {HomeDataLanguage.sponsors_desc(currentLanguage)}
            </p>
          </div>
          <Swiper
            slidesPerView={3}
